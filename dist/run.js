...
import { writeFinalReport, } from './deep-research.js';
...
async function run() {
    try {
        ...
        let breadth;
        const breadthInput = await askQuestion('Enter research breadth (recommended 2-10, default 4): ');
        breadth = Number.parseInt(breadthInput, 10);
        breadth = isNaN(breadth) ? 4 : Math.max(1, Math.min(10, breadth)); // Default 4, range 1-10
        let depth;
        const depthInput = await askQuestion('Enter research depth (recommended 1-5, default 2): ');
...
`;
        ...
        log(`\nAn error occurred during the research process: ${error instanceof Error ? error.message : String(error)}`); // Log error to output manager as well
    }
    finally {
